/**
 * The progress spinner, rendered by `ora`.
 *
 * It writes to stderr (so stdout stays clean for piping) and only animates
 * when explicitly enabled: callers gate on `process.stderr.isTTY` and
 * `--no-spinner`, and ora's own TTY/CI detection is bypassed so that decision
 * lives in one place. When disabled, `start`/`update` are silent no-ops but
 * final status lines (`succeed`/`fail`) still print. This keeps the progress
 * indicator out of the way of `aicmd --dry-run "..." | sh`.
 *
