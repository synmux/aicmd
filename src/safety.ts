...
const DEFAULT_PATTERN_TABLE: ReadonlyArray<{ pattern: string; label: string }> =
  [
    ...
    {
      pattern:
        "re:\\b(?:curl|wget)\\b.*\\|\\s*(?:sudo\\s+)?(?:ba|z|da|fi)?sh\\b",
...
export const DEFAULT_DANGEROUS_PATTERNS: readonly string[] =
  DEFAULT_PATTERN_TABLE.map((entry) => entry.pattern);
  ...
  if (pattern.startsWith(REGEX_PREFIX)) {
    ...
    try {
      ...
      throw new AicmdError(
        `Invalid dangerous-command pattern "${source}": ${(err as Error).message}`,
      );
    }
  }
...
export function effectivePatterns(
  config: Pick<Config_, "dangerousPatterns" | "extraDangerousPatterns">,
): string[] {
...
export function assessDanger(
  ...
  if (matched !== null) {
    ...
    reasons.push(
      label !== undefined
        ? `matches guard pattern: ${label}`
        : `matches guard pattern "${matched}"`,
    );
  }
  if (candidate.dangerous) {
    ...
    reasons.push(
      reason && reason.length > 0
        ? reason
        : "The model flagged this command as dangerous.",
    );
  }
  return {
