...
export function buildProgram(): Command {
  ...
  program
    .name("aicmd")
    .description(
      "Generate a shell command from a natural-language task with Claude, " +
        "confirm it, and run it.",
    )
    ...
    .option(
      "-x, --execute",
      "run the command without a confirmation prompt (dangerous commands are still refused)",
    )
    .option(
      "-f, --force",
      "with --execute: run even when the command is flagged dangerous",
    )
    .option("-d, --dry-run", "print the command to stdout and exit")
    .option(
      "-i, --interactive",
      "pick between several candidate commands in an interactive TUI",
    )
    .option(
      "--no-interactive",
    ...
    .option(
      "-n, --count <n>",
      "number of candidates to generate in interactive mode",
      parseCount,
    )
    .option("-m, --model <model>", "model to use (alias or full model id)")
    .option(
      "-s, --shell <shell>",
      "shell to generate for and execute with (default: $SHELL)",
    )
    .option("-p, --prompt <text>", "extra instructions appended to the prompt")
    .option(
      "--no-context",
      "omit the platform/shell context block from the prompt",
    )
...
export type RunMode =
  "dry-run" | "execute" | "interactive" | "confirm" | "print" | "no-tty-error";
...
export type PlainAction =
  "refuse-dangerous" | "refuse-unassessed" | "run" | "print" | "confirm";
...
export function printDangerWarning(danger: DangerAssessment): void {
  process.stderr.write(
    color("33", "⚠  This command looks potentially destructive:") + "\n",
  );
...
export async function run(argv: string[]): Promise<number> {
  ...
  try {
    ...
    if (task === "") {
      throw new AicmdError(
        "No task given. Pass it as arguments, pipe it on stdin, or type it at the prompt.",
      );
    }
...
async function runPlain(
  ...
  const spinner = new Spinner(
    flow.spinnerEnabled && process.stderr.isTTY,
    config.spinner,
  );
  ...
  switch (action) {
    case "print": {
      ...
      if (flow.mode === "print") {
        process.stderr.write(
          color("90", "No interactive terminal: printing instead of running.") +
            "\n",
        );
      }
    ...
    case "refuse-dangerous": {
      process.stderr.write(
        color("31", "✖ SAFETY:") +
          " refusing to auto-execute a potentially destructive command.\n",
      );
      printDangerWarning(danger);
      printCommandBlock(candidate.command);
      process.stderr.write(
        "Re-run with --force (-f) to execute anyway, or without --execute " +
          "(-x) for a confirmation prompt.\n",
      );
      return 1;
    }
    ...
    case "run": {
      if (danger.dangerous) {
        process.stderr.write(
          color("33", "⚠  FORCE MODE: executing despite the danger flags.") +
            "\n",
        );
        printDangerWarning(danger);
      }
    ...
    case "confirm": {
      ...
      if (choice === "edit") {
        ...
        const editedDanger = assessDanger(
