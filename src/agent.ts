 ...
 * single text (or structured JSON) completion.
 *
 ...
 * beyond the prompt we build, and the model can never execute anything while
 * "only writing a command".
 */
...
export interface RunPromptOptions {
   ...
   * object is returned on {@link ModelResult.structured}. Models that don't
   * support structured outputs will reject the request, so the caller should
   * be prepared to retry without it.
   */
...
/**
 * Build the Agent SDK options for one isolated, single-turn completion.
 *
 ...
 * on a busy setup) into every request. For aicmd the `tools: []` line is
 * also a safety property: the model writes a command but can never run one.
 */
export function buildQueryOptions(opts: RunPromptOptions, subprocessEnv?: Record<string, string | undefined>): Options {
  return {
    model: opts.model,
    systemPrompt: opts.system,
    tools: [], // pure completion: no Bash/Read/Edit/etc.
    skills: [],
    mcpServers: {},
...
/**
 * Run a single prompt and return the model's response.
 *
 * Throws {@link AicmdError} on any model/authentication/quota failure.
 */
export async function runPrompt(prompt: string, opts: RunPromptOptions): Promise<ModelResult> {
  ...
  try {
      ...
      switch (message.type) {
        ...
        case "result": {
          ...
          if (message.subtype === "success") {
            ...
            throw new AicmdError(`Model run failed: ${detail}`);
          }
          break;
        }
        default:
    ...
    if (opts.abortController?.signal.aborted) {
      throw new AicmdError("Generation was cancelled.");
    }
    throw new AicmdError(`Failed to call the Claude Agent SDK: ${(err as Error).message}`);
  }

  if (assistantError) {
    throw new AicmdError(describeAssistantError(assistantError));
  }
  if (resultText === null) {
    throw new AicmdError("The model returned no result.");
  }
