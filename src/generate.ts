...
export async function generateCommands(
  ...
  progress.onPhase?.(
    count > 1 ? `Generating ${count} command options` : "Generating command",
  );
  ...
  for (const attempt of attempts) {
    try {
      ...
      candidates = attempt.structured
        ? extractCandidates(result.structured)
        : plainToCandidates(result.text, count);
