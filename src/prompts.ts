      ...
      items: {
        ...
        properties: {
          command: {
            type: "string",
            description:
              "One single-line shell command that accomplishes the task.",
          },
          explanation: {
            type: "string",
            description:
...
export function extractCandidates(
  ...
  for (const entry of entries) {
    ...
    candidates.push({
      ...
      modelAssessed: true,
      ...(typeof record.dangerReason === "string" &&
      record.dangerReason.trim() !== ""
...
export function buildSystem(opts: SystemPromptOptions): string {
  const rules: string[] = [
    ...
    "Do not perform the task yourself and do not ask questions - only write " +
      "the command.",
...
export function buildUser(
  ...
  if (count <= 1) {
    return structured
      ? `${header}\n\nProduce one command for this task and return it as the ` +
          `only element of the "commands" array.`
      : header;
  }
 ...
 * the safety net.
 */
export function plainToCandidates(
  text: string,
  count: number,
): GeneratedCommand[] {
