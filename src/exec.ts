...
export function signalExitCode(signal: NodeJS.Signals): number {
  const numbers = osConstants.signals as unknown as Record<
    string,
    number | undefined
  >;
...
export async function executeCommand(
  ...
  return await new Promise<number>((resolveExit, reject) => {
    ...
    const priorSigintListeners = process.rawListeners("SIGINT") as Array<
      (...args: unknown[]) => void
    >;
    ...
    child.on("error", (err) => {
      cleanup();
      reject(
        new AicmdError(`Could not run the shell ${shellPath}: ${err.message}`),
      );
    });
