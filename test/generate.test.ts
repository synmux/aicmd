...
describe("generateCommands", () => {
  ...
  test("returns candidates from a structured response", async () => {
    ...
    expect(result.candidates[0]!.command).toBe("ls -la");
    expect(result.costUsd).toBe(0.01);
    expect(result.model).toBe("claude-sonnet-5");

    expect(calls).toHaveLength(1);
    expect(calls[0]!.opts.outputFormat).toEqual({
      type: "json_schema",
      schema: COMMANDS_SCHEMA,
    });
    expect(calls[0]!.opts.system).toContain("Platform: test.");
    expect(calls[0]!.prompt).toContain("list files");
  });
  ...
  test("falls back to plain text when structured attempts fail", async () => {
    ...
    expect(result.candidates[0]!.modelAssessed).toBe(false);
    expect(calls).toHaveLength(2);
    expect(calls[0]!.opts.outputFormat).toBeDefined();
    expect(calls[1]!.opts.outputFormat).toBeUndefined();
  });
  ...
  test("passes a verbose stderr sink through to the runner", async () => {
    ...
    expect(calls[0]!.opts.onStderr).toBe(sink);
  });
  ...
  test("a malformed structured payload falls through to the next attempt", async () => {
    ...
    expect(result.candidates[0]!.command).toBe("du -sh *");
    expect(calls).toHaveLength(2);
  });
  ...
  test("count > 1 with a temperature tries temperature first, then retries without", async () => {
    ...
    expect(calls[0]!.opts.temperature).toBe(config.interactiveTemperature!);
  ...
  test("dedupes identical commands", async () => {
    const { runner } = scriptedRunner([
      structuredResult([candidate, { ...candidate, explanation: "again" }]),
    ]);
    const result = await generateCommands("list files", config, {
      runner,
      count: 2,
  ...
  test("throws an AicmdError when every attempt fails", async () => {
    const { runner } = scriptedRunner([
      new AicmdError("model exploded"),
      new AicmdError("model exploded again"),
    ]);
    await expect(
      generateCommands("list", config, { runner, shellPath: "/bin/sh" }),
    ).rejects.toThrow("model exploded again");
  });
  ...
  test("passes allowApiKey through to the runner", async () => {
    ...
    await generateCommands(
      ...
      { runner, shellPath: "/bin/sh" },
    );
    expect(calls[0]!.opts.allowApiKey).toBe(true);
  });
});
