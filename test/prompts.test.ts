...
describe("buildSystem", () => {
  ...
  test("includes the context block when provided", () => {
    ...
    expect(
      buildSystem({ ...baseSystemOpts, contextBlock: null }),
    ).not.toContain("Platform: macOS.");
  });
...
describe("COMMANDS_SCHEMA and extractCandidates", () => {
  test("schema requires command, explanation and dangerous", () => {
    const items = (
      COMMANDS_SCHEMA.properties as Record<string, Record<string, unknown>>
    ).commands!.items as Record<string, unknown>;
  ...
  test("extracts well-formed candidates", () => {
    ...
    expect(out![1]!.dangerous).toBe(true);
    expect(out![1]!.dangerReason).toBe("deletes files");
  });
  ...
  test("rejects malformed shapes", () => {
    ...
    expect(
      extractCandidates({ commands: [{ explanation: "x", dangerous: false }] }),
    ).toBeNull();
  });
  ...
  test("tolerates a missing dangerous flag by treating it as false", () => {
    ...
    expect(out![0]!.dangerous).toBe(false);
  });
  ...
  test("marks structured candidates as model-assessed", () => {
    ...
    expect(out![0]!.modelAssessed).toBe(true);
  });
  ...
  test("rejects commands smuggling control characters", () => {
    ...
    expect(
      extractCandidates({
        commands: [
          { command: "ls [8mhidden[0m", explanation: "x", dangerous: false },
        ],
      }),
    ).toBeNull();
  });
...
describe("cleanCommand", () => {
  ...
  test("keeps interior quotes intact", () => {
    expect(cleanCommand("grep 'foo bar' file.txt")).toBe(
...
describe("plainToCandidates", () => {
  test("a single plain response becomes one candidate with unknown danger", () => {
    ...
    expect(out[0]!.modelAssessed).toBe(false);
  });
