...
describe("sanitizePartial", () => {
  ...
  test("shell and customPrompt accept explicit null", () => {
    ...
    expect(sanitizePartial({ customPrompt: "be terse" }).customPrompt).toBe(
      "be terse",
    );
  });
  ...
  test("interactiveTemperature accepts null, clamps to 0..2, ignores bad types", () => {
    ...
    expect(
      sanitizePartial({ interactiveTemperature: 0.7 }).interactiveTemperature,
    ).toBe(0.7);
    expect(
      sanitizePartial({ interactiveTemperature: 9 }).interactiveTemperature,
    ).toBe(2);
    expect(
      sanitizePartial({ interactiveTemperature: -3 }).interactiveTemperature,
    ).toBe(0);
    expect(
  ...
  test("dangerousPatterns accepts null or a string array, filtering non-strings", () => {
    expect(
      sanitizePartial({ dangerousPatterns: null }).dangerousPatterns,
    ).toBeNull();
    expect(
      sanitizePartial({ dangerousPatterns: ["*rm *", 7, "re:x"] })
        .dangerousPatterns,
    ).toEqual(["*rm *", "re:x"]);
    expect(
      sanitizePartial({ dangerousPatterns: "x" }).dangerousPatterns,
    ).toBeUndefined();
  });

  test("extraDangerousPatterns accepts a string array only", () => {
    expect(
      sanitizePartial({ extraDangerousPatterns: ["*x*"] })
        .extraDangerousPatterns,
    ).toEqual(["*x*"]);
    expect(
      sanitizePartial({ extraDangerousPatterns: null }).extraDangerousPatterns,
    ).toBeUndefined();
  });
  ...
  test("unknown spinner names are dropped", () => {
    ...
    expect(
      sanitizePartial({ spinner: "definitely-not-real" }).spinner,
    ).toBeUndefined();
  });
});
...
describe("merge and precedence", () => {
  ...
  test("mergePartial is a flat overlay", () => {
    expect(
      mergePartial({ model: "a" }, { model: "b", interactive: true }),
    ).toEqual({
      model: "b",
      interactive: true,
    });
...
describe("loadFileConfig", () => {
  ...
  test("reads .aicmdrc.json and .aicmdrc", async () => {
    ...
    await mkdir(nestedB);
    await writeFile(
      join(nestedA, ".aicmdrc.json"),
      JSON.stringify({ model: "opus" }),
    );
    await writeFile(
      join(nestedB, ".aicmdrc"),
      JSON.stringify({ model: "haiku" }),
    );
  ...
  test("walks up from cwd to stopDir to find a config", async () => {
    ...
    await writeFile(
      join(dir, ".aicmd.json"),
      JSON.stringify({ interactive: true }),
    );
  ...
  test("reads the aicmd key from the nearest package.json that has one", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ aicmd: { model: "haiku" } }),
    );
  ...
  test("a dedicated config file overrides package.json", async () => {
    await writeFile(
      join(dir, "package.json"),
      JSON.stringify({ aicmd: { model: "haiku", interactive: true } }),
    );
    await writeFile(
      join(dir, ".aicmd.json"),
      JSON.stringify({ model: "opus" }),
...
describe("global config (XDG)", () => {
  ...
  test("globalConfigDir falls back to ~/.config when XDG is unset or relative", () => {
    ...
    expect(globalConfigDir({ XDG_CONFIG_HOME: "relative/path" })).toMatch(
      /\.config\/aicmd$/,
    );
  });
  ...
  test("project config overrides the global config", async () => {
    await writeFile(
      join(globalDir(), "config.json"),
      JSON.stringify({ model: "haiku", interactive: true }),
    );
    await writeFile(
      join(projectDir, ".aicmd.json"),
      JSON.stringify({ model: "opus" }),
    );
