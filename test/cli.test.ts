...
describe("resolveRunMode", () => {
  ...
  test("--dry-run always wins", () => {
    expect(resolveRunMode({ ...base, dryRun: true, execute: true })).toBe(
      "dry-run",
    );
    expect(
      resolveRunMode({ ...base, dryRun: true, configInteractive: true }),
    ).toBe("dry-run");
  });

  test("--execute runs without confirmation, TTY or not", () => {
    expect(resolveRunMode({ ...base, execute: true })).toBe("execute");
    expect(resolveRunMode({ ...base, execute: true, hasTty: false })).toBe(
      "execute",
    );
  });
  ...
  test("--execute beats interactive", () => {
    expect(
      resolveRunMode({ ...base, execute: true, configInteractive: true }),
    ).toBe("execute");
  });

  test("interactive from config or flag needs a TTY", () => {
    expect(resolveRunMode({ ...base, configInteractive: true })).toBe(
      "interactive",
    );
  });
  ...
  test("config-driven interactive without a TTY quietly prints", () => {
    expect(
      resolveRunMode({ ...base, configInteractive: true, hasTty: false }),
    ).toBe("print");
  });
...
describe("flagsToConfig", () => {
  ...
  test("floors and clamps the count", () => {
    ...
    expect(
      flagsToConfig({ count: Number.NaN }).interactiveCount,
    ).toBeUndefined();
  });
...
describe("decideAction", () => {
  ...
  test("execute refuses when the model's danger signal was unavailable", () => {
    ...
    expect(decideAction({ ...base, modelSignalAvailable: false })).toBe(
      "refuse-unassessed",
    );
    expect(
      decideAction({ ...base, modelSignalAvailable: false, force: true }),
    ).toBe("run");
  });

  test("dry-run and print always print, dangerous or not", () => {
    expect(decideAction({ ...base, mode: "dry-run", dangerous: true })).toBe(
      "print",
    );
    expect(decideAction({ ...base, mode: "print", dangerous: true })).toBe(
      "print",
    );
  });

  test("confirm mode always confirms", () => {
    expect(decideAction({ ...base, mode: "confirm", dangerous: true })).toBe(
      "confirm",
    );
  });
});
...
describe("taskFromArgs", () => {
  test("joins argv words into one task string", () => {
    expect(taskFromArgs(["list", "the", "five", "largest", "files"])).toBe(
