...
describe("pickerEntryLabel", () => {
  test("prefixes dangerous commands with a warning marker", () => {
    expect(pickerEntryLabel(entry("rm -rf /tmp/x", "removes", true))).toBe(
      "⚠ rm -rf /tmp/x",
    );
...
describe("buildPickerScene", () => {
  const entries: PickerEntry[] = [
    ...
    entry("rm -rf ./cache", "clears the cache", true, [
      "matches guard pattern",
    ]),
  ];
  ...
  test("keeps the picker visible on a small terminal", async () => {
    ...
    expect(frame).toContain(entries[0]!.candidate.command);
  });
